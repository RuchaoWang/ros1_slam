...
# 3) 启动 Gazebo + robot
gnome-terminal -t "gazebo_start" -- bash -lc "source ~/Gitkraken/ros1_noetic_nav/devel/setup.bash; roslaunch tri_steer_gazebo sim_gazebo_rviz.launch"
...
# 5）启动地图节点
gnome-terminal -t "map start" -- bash -lc "source ~/Gitkraken/ros1_noetic_nav/devel/setup.bash; roslaunch robot_costmap map_deal.launch;exec bash"

# 6) 运动规划节点
gnome-terminal -t "motion Plan start" -- bash -lc "source ~/Gitkraken/ros1_noetic_nav/devel/setup.bash; roslaunch robot_navigation motionPlan_sim.launch"

# 7) 雷达点云转换成point-cloud2
gnome-terminal -t "laser to cloud" -- bash -lc \
"source ~/Gitkraken/ros1_noetic_nav/devel/setup.bash; 
...
# 10）订阅控制消息
gnome-terminal -t "control  start" -- bash -lc "source ~/Gitkraken/ros1_noetic_nav/devel/setup.bash; roslaunch tri_steer_drive bringup.launch use_teleop:=true;exec bash"
